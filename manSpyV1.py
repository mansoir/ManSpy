  ...
  \ \  \    \ \  \ \  \ \  \ \  \\ \  \|____|\  \ \  \___|\/  /  /  
   \ \__\    \ \__\ \__\ \__\ \__\\ \__\____\_\  \ \__\ __/  / /    
    \|__|     \|__|\|__|\|__|\|__| \|__|\_________\|__||\___/ /     
                     ...
                     """+bcolors.ENDC+"""(Mansour Eddih 4isi)
                            """)
        print (bcolors.ENDC)
...
class Spinner:
    ...
    def spinner_task(self):
        try:
            ...
            #clear()
            print ("\n\t"+ bcolors.BG_ERR_TXT+"ManSpy received a series of Ctrl+C hits. Quitting..." +bcolors.ENDC)
            sys.exit(1)
...
if len(sys.argv) == 1 :
    ...
    if target == '--update' or target == '-u' or target == '--u':
    	logo()
        print ("ManSpy is updating....Please wait.\n")
        spinner.start()
        ...
        if rs_internet_availability == 0:
            ...
            spinner.stop()
            sys.exit(1)
        cmd = 'sha1sum ManSpy.py | grep .... | cut -c 1-40'
        oldversion_hash = subprocess.check_output(cmd, shell=True)
        oldversion_hash = oldversion_hash.strip()
        os.system('wget -N https://raw.githubusercontent.com/skavngr/ManSpy/master/ManSpy.py -O ManSpy.py > /dev/null 2>&1')
        ...
        if oldversion_hash == newversion_hash :
            clear()
            print "\t"+ bcolors.OKBLUE +"You already have the latest version of manSpy." + bcolors.ENDC
        else:
            clear()
            print "\t"+ bcolors.OKGREEN +"ManSpy successfully updated to the latest version." +bcolors.ENDC
        spinner.stop()
        sys.exit(1)
        ...
        while (rs_avail_tools < len(tools_precheck)):
			...
			try:
				...
				print "\t"+bcolors.BG_ERR_TXT+"ManSpy was terminated abruptly..."+bcolors.ENDC
				sys.exit(1)
			if "not found" in val:
        ...
        if unavail_tools == 0:
        	print "\t"+bcolors.OKGREEN+"All Scanning Tools are available. All vulnerability checks will be performed by ManSpy."+bcolors.ENDC
        else:
        	print "\t"+bcolors.WARNING+"Some of these tools "+bcolors.BADFAIL+str(unavail_tools_names)+bcolors.ENDC+bcolors.WARNING+" are unavailable. ManSpy can still perform tests by excluding these tools from the tests. Please install these tools to fully utilize the functionality of ManSpy."+bcolors.ENDC
		...
		rs_skipped_checks = rs_skipped_checks + 1
            ...
            if runTest == 1:
                    ...
                    clear()
                    print "\t"+bcolors.WARNING + "Test Skipped. Performing Next. Press Ctrl+Z to Quit ManSpy." + bcolors.ENDC
                    rs_skipped_checks = rs_skipped_checks + 1
        ...
        if len(rs_vul_list)==0:
        	...
        	with open("RS-Vulnerability-Report", "a") as report:
        		while(rs_vul < len(rs_vul_list)):
	       		...
	       		print "\tComplete Vulnerability Report for "+bcolors.OKBLUE+target+bcolors.ENDC+" named "+bcolors.OKGREEN+"`RS-Vulnerability-Report`"+bcolors.ENDC+" is available under the same directory ManSpy resides."
